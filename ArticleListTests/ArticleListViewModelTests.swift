...
class ArticleListViewModelTests: XCTestCase {
    ...
    func testGetArticleCount() {
        let sut = ArticleListViewModel(articleService: MockArticleService(status: .success), userDefault: MockUserDefaults(suiteName: "testing")!)

        XCTAssertEqual(10, sut.getArticleCount())
    }
}
